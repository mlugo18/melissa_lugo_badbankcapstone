//DEPOSIT REFACTORED 5.23 - pulls balance from mongodb and updates balance with deposited amount
      ...
      <div>
          <Card
              ...
              body={show ? (
                      ...
                      <br/>
                      <input type="number" className="form-control" placeholder="Enter deposit amount" value={amount} onChange={e => setAmount(e.currentTarget.value)}/><br/>
                      <button type="submit" className="btn btn-light" onClick={() => handle(authUser.email)}>Deposit</button>
                      </>
                    ) : (
                      <>
                      Account Balance: {formatCurrency(balance)} <br/>
                      <div> Successfully deposited! </div>
                      <button type="button" className="btn btn-light" onClick={clearForm}>Add another deposit</button>
                      </>
                    )
                  }
            />
